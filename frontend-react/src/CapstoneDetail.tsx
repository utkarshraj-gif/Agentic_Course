...
import { Code, Copy, ChevronLeft } from 'lucide-react';
...
export const CapstoneDetail = () => {
  ...
  if (!data) return <div className="main-content">Loading capstone details...</div>;
  ...
  return (
    <div className="main-content" style={{ padding: '0' }}>
      ...
      <div style={{ padding: '2rem 4rem', backgroundColor: 'var(--color-bg-light)', borderBottom: '1px solid var(--color-border)' }}>
        ...
        <span className="course-tag">{data.domain}</span>
        <h1 style={{ margin: '0.5rem 0', fontSize: '2.2rem' }}>{data.title}</h1>
        <div style={{ display: 'flex', gap: '2rem', marginTop: '1rem', color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
          <div><strong>Pattern:</strong> {data.pattern}</div>
          <div><strong>Metric:</strong> {data.metric}</div>
        </div>
      </div>
